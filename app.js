...
,{name:'Möbius Corridor',desc:'Walk the loop · orientation flips',seedGlyphs:'זאב',start:{x:2.5,y:2.5,dir:0},glitch:false,
 rooms:[{id:'ז',x:1,y:1,w:11,h:3},{id:'א',x:1,y:7,w:11,h:3}],
 map:['1111111111111','1000000000001','1000000000001','1000000000001','1110111010111','1000000000001','1110111010111','1000000000001','1000000009001','1000000000001','1111111111111'],
 portalCells:[[1,2,2],[11,2,3],[1,8,4],[11,8,5],[6,4,6],[6,6,7]],falseWalls:[[3,5],[9,5]],duckCell:[10,1],
 portals:{2:{x:1.5,y:2.5,partner:4,facing:Math.PI,op:'invert',glyph:'ז'},4:{x:1.5,y:8.5,partner:2,facing:0,op:'invert',glyph:'ז'},3:{x:11.5,y:2.5,partner:5,facing:0,op:'rotate',glyph:'א'},5:{x:11.5,y:8.5,partner:3,facing:Math.PI,op:'rotate',glyph:'א'},6:{x:6.5,y:4.5,partner:7,facing:Math.PI/2,op:'fold',glyph:'ג'},7:{x:6.5,y:6.5,partner:6,facing:-Math.PI/2,op:'fold',glyph:'ג'}}}
,{name:'Bigger Inside',desc:'Small outer shell · large inner via portals',seedGlyphs:'דהו',start:{x:2.5,y:2.5,dir:0},glitch:false,
 rooms:[{id:'ד',x:1,y:1,w:3,h:3},{id:'ה',x:5,y:1,w:7,h:5},{id:'ו',x:1,y:7,w:3,h:3}],
 map:['1111111111111','1000100000001','1000100000001','1000100000001','1110111110111','1000000000001','1110111110111','1000100000001','1000100009001','1000100000001','1111111111111'],
 portalCells:[[3,2,2],[5,2,3],[3,8,4],[5,8,5],[7,4,6],[7,6,7]],falseWalls:[[9,2],[9,8]],duckCell:[10,5],
 portals:{2:{x:3.5,y:2.5,partner:3,facing:0,op:'translate',glyph:'ד'},3:{x:5.5,y:2.5,partner:2,facing:Math.PI,op:'translate',glyph:'ד'},4:{x:3.5,y:8.5,partner:5,facing:0,op:'bridge',glyph:'ו'},5:{x:5.5,y:8.5,partner:4,facing:Math.PI,op:'bridge',glyph:'ו'},6:{x:7.5,y:4.5,partner:7,facing:Math.PI/2,op:'split',glyph:'ה'},7:{x:7.5,y:6.5,partner:6,facing:-Math.PI/2,op:'split',glyph:'ה'}}}
,{name:'Shuk Loop',desc:'Market corridors · two portal pairs',seedGlyphs:'בגדה',start:{x:2.5,y:1.5,dir:Math.PI/2},glitch:false,
 rooms:[{id:'ב',x:1,y:1,w:4,h:2},{id:'ג',x:8,y:1,w:4,h:2},{id:'ד',x:1,y:5,w:4,h:3},{id:'ה',x:8,y:5,w:4,h:3}],
 map:['1111111111111','1000100010001','1000100010001','1110111010111','1000000000001','1000100010001','1000100010001','1000100010001','1110111010111','1000000009001','1111111111111'],
 portalCells:[[4,1,2],[8,1,3],[4,6,4],[8,6,5],[2,3,6],[2,4,7],[10,3,10],[10,4,11]],falseWalls:[[6,2],[6,7]],duckCell:[6,9],
 portals:{2:{x:4.5,y:1.5,partner:3,facing:0,op:'mirror',glyph:'ב'},3:{x:8.5,y:1.5,partner:2,facing:Math.PI,op:'mirror',glyph:'ב'},4:{x:4.5,y:6.5,partner:5,facing:0,op:'fold',glyph:'ג'},5:{x:8.5,y:6.5,partner:4,facing:Math.PI,op:'fold',glyph:'ג'},6:{x:2.5,y:3.5,partner:7,facing:Math.PI/2,op:'translate',glyph:'ד'},7:{x:2.5,y:4.5,partner:6,facing:-Math.PI/2,op:'translate',glyph:'ד'},10:{x:10.5,y:3.5,partner:11,facing:Math.PI/2,op:'bridge',glyph:'ו'},11:{x:10.5,y:4.5,partner:10,facing:-Math.PI/2,op:'bridge',glyph:'ו'}}}
...
const keys={};
addEventListener('keydown',e=>{const k=e.key.toLowerCase();keys[k]=true;if(!started)startGame();if(k==='m')showMinimap=!showMinimap;if(k==='v')ANAGLYPH=!ANAGLYPH;if(k==='g')togglePanel('obsPanel');if(k==='t')togglePanel('topoPanel');if(k>='1'&&k<='9'){const n=parseInt(k,10)-1;if(n<LEVEL_DATA.length){loadLevel(n);if(!started)startGame();}}});
...
function tryPortal(){
  ...
  if(pid&&PORTALS[pid]){const src=PORTALS[pid],dst=PORTALS[src.partner];if(dst){const from=currentRoom;const tr=portalExitRay(src,dst,Math.cos(player.dir),Math.sin(player.dir));player.dir=tr.ang;player.x=dst.x+tr.rdx*0.45;player.y=dst.y+tr.rdy*0.45;portalCooldown=0.55;logGlyph(src.glyph||dst.glyph,src.op||dst.op,from);portalHistory.push({glyph:src.glyph||'?',op:src.op||dst.op,fromRoom:from});updateHistoryUI();currentRoom=roomAt(player.x,player.y);showRoomBanner(currentRoom,(src.op||'PORTAL').toUpperCase()+' · '+ (src.glyph||''));}}}
